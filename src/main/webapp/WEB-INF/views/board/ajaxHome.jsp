<%@ page contentType="text/html; charset=UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!DOCTYPE html>
<html>
<head>
<script type="text/javascript" src="/resources/js/jquery.min.js"></script>
<title>Header 매핑</title>
<script type="text/javascript">
	$(function(){
		//document가 로딩이 완료 되면 실행
		$("button").on("click",function(){
			let boardNoval = "7";
			let boardObject = {
				"boardNo" 	: "7",
				"title"		: "개똥이 수리남 가다",
				"content"	: "식사는 잡쉈어?",
				"writer"	: "개똥이"
			};
			console.log("boardObject:",boardObject);
			console.log("boardObject 변환: ",JSON.stringify(boardObject))
			//contentType => 보내는 타입
			//dataType => 응답타입
			$.ajax({
				url:"/board/"+boardNoval,
				headers:{
					"X-HTTP-Method-Override":"PUT"
				},
				contentType: "appliction/json;charset=utf-8",
				data: JSON.stringify(boardObject),
				type: "put",
				success:function(result){
					console.log("result",result)
				}
				
			});
			
		});
		
		$("input[name='btnAccept']").on("click",function(){
			let boardNo = $("#boardNo").val();	//7
			console.log(boardNo);
			//get방식으로 /board/7 Uri를 요청하면
			//JSON 데이터로 비동기 응답이 되겠구나
			$.get("/board/"+boardNo,function(data){
				//data : Json데이터
				console.log("data: ",JSON.stringify(data));
			})
		});
		
		$("input[name='btnJson']").on("click",function(){
			let boardNo = $("#boardNo").val();
			console.log("boardNo:"+boardNo)
			let data={
				"boardNo":boardNo
			}
			$.ajax({
				url: "/board/getBook",
				contentType:"application/json;charset:utf-8",
				data:JSON.stringify(data),
				type:"post",
				success:function(result){
					//result는 bookVO데이터가 json변환 되서 옴
					console.log("result: ",result);
				}
			});
			
		});
	});
</script>
</head>
<body>
	<h2>Header 매핑</h2>
	<button type="button">식사</button>
	<h2>7.Accept 매핑</h2>
	<input type="text" name="boardNo" id="boardNo" value="7">
	<input type="button" name="btnAccept" value="실행">
	<input type="button" name="btnJson" value="ajax실행">
</body>
</html>